{ 
  ...
  plot1 = ggplot() +
    geom_sf(data = states_SF,
            mapping = aes(geometry = geometry, size=datarank)) +
    geom_sf(data = lakes_SF,
            mapping = aes(geometry = geometry, shape=admin),
            color = "lightblue",
            fill = "lightblue") +
    geom_sf_label(data = museums_SF,
             mapping = aes(geometry=geometry, color=Presidential.Library, label=substr(museum,
                                                           start=1,
                                                           stop=3))) +
  ...
  plot4 = plot2b +
    scale_color_manual(values=c(museums_SF$Presidential.Library),
                       Yes = "red",
                       No = "green")
  plot(plot4b)
